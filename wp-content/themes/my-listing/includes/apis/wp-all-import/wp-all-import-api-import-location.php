...
function import_location( $field, $field_value, $log ) {
	...
	if ( $method === 'address' ) {
		...
		try {
			...
			update_post_meta( $field->listing->get_id(), '_'.$field->get_key(), $address );
			update_post_meta( $field->listing->get_id(), 'geolocation_lat', $feature['latitude'] );
			update_post_meta( $field->listing->get_id(), 'geolocation_long', $feature['longitude'] );
		} catch ( \Exception $e ) {
			return $log( sprintf(
	...
	if ( $method === 'coordinates' ) {
		...
		try {
			...
			update_post_meta( $field->listing->get_id(), '_'.$field->get_key(), $feature['address'] );
			update_post_meta( $field->listing->get_id(), 'geolocation_lat', $latitude );
			update_post_meta( $field->listing->get_id(), 'geolocation_long', $longitude );
		} catch ( \Exception $e ) {
			return $log( sprintf(
